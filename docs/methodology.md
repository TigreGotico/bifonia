...
### LLM-assisted expansion (guided and reviewed)

After the human seed, a large-language model (Claude Sonnet 4.6, via the Anthropic API)
generated additional sentences in batches.  The generation process was:
...
2. The LLM generated candidate sentences.
...
| LLM-assisted (human-guided) | ~10 500 |
| Rule-based accuracy on full corpus | **99.51 %** |
...
- **LLM usage:** approximately 95 % of corpus sentences were generated by Claude Sonnet
  4.6 under close human supervision.  No sentence was admitted to the corpus without
  passing the scorer (and any necessary rule fix).  The human seed (~500 sentences)
  defines the disambiguation gold standard; LLM sentences expand coverage of hard patterns.
